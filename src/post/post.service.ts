...
export class PostService {
    ...
    async create(createPostInput: CreatePostInput) {
        ...
        const { inserted, changes } = await this.rethinkService.saveDB('postMessages', post);
        if (inserted) {
    ...
    async update(updatePostInput: UpdatePostInput) {
        ...
        const { replaced, changes } = await this.rethinkService.updateDB('postMessages', id, updatePostInput);
        if (replaced) {
            return changes[0].new_val;
        } else {
